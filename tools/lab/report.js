...
/* 보스와 방침 목록은 measure 가 쥔다 — 여기 또 적으면 씨앗 목록처럼 조용히 갈린다 */
const { BOSSES, POLS } = require('./measure.js');
...
function table(vars) {
  ...
  for (const boss of BOSSES) {
    /* 한 보스의 값 한 벌은 판본마다 한 번만 잰다. 전에는 지표 줄마다 · 장부 줄마다
       stat 을 다시 불러 같은 540판을 스무 번 되짚었다 */
    const S = {}; for (const v of vars) S[v] = stat(D[v], boss);
    ...
    for (const [k, f] of KEYS)
      console.log('  ' + k.padEnd(14) + vars.map(v => f(S[v][k]).padStart(12)).join(''));
    console.log('  ── 방침별 여백 ──');
    for (const p of POLS)
      console.log('    ' + p.padEnd(12) + vars.map(v => f2(stat(D[v], boss, p).여백).padStart(12)).join(''));
    console.log('  ── 증상별 피해 장부 (%) ──');
    const keys = new Set(); for (const v of vars) for (const k in S[v].장부) keys.add(k);
    for (const k of [...keys].sort())
      console.log('    ' + k.padEnd(12) + vars.map(v => (S[v].장부[k] ?? 0).toFixed(1).padStart(12)).join(''));
  }
}
...
function spots(vars) {
  console.log('\n━━ 자리가 갖고 태어난 진화 카운터 ━━');
  const D = {}; for (const v of vars) D[v] = load(v).rows;   // 보스마다 다시 읽지 않는다
  for (const boss of BOSSES) {
    console.log(`\n  ${boss}`);
    for (const v of vars) {
      const rows = D[v].filter(r => r.boss === boss);
      const agg = {};
