...
/* 사혈은 「지금 아픈 값으로 나중을 산다」는 거래다. 이 카드는 그 거래를 가장
   벌거벗은 꼴로 낸다 — 코스트가 0 이라 치르는 것이 오직 피뿐이다.
   목표를 안 고르므로 낼지 말지만 정한다 (소독하겠습니다와 같은 잣대) */
'피를 본다': {cost:0, dept:'외과', verb:'보조', kw:'사혈', bleed:1, target:'none', v:{draw:2},
  text:'사혈 {bleed}단 · {draw}장 뽑는다',
  fx:(S,n,a,v)=>draw(S,v.draw)},
...
/* 순수 지연 하나. 「손이 기억한다」(NPC)는 억제에 지연을 얹어 주지만 이쪽은
   미루기만 한다 — 그래서 값이 더 크다 (1턴 → 2턴).
   지연은 공짜가 아니다: 미룬 턴 1당 자리가 **지금 수치**의 10% 씩 자란다 (커널.지연).
   깎아 둔 자리에 걸면 싸고, 부푼 자리에 걸면 비싸다 — 언제 미루느냐가 판단이다.
   그 대가를 본문에 글로 박지 않는다 — 규칙값이 바뀌면 그 설명만 옛말이 된다.
   키워드 「지연」 툴팁이 지금 값으로 말한다 */
'하루만 더': {cost:1, dept:'내과', verb:'보조', kw:'지연', target:'node', v:{delay:2},
  text:'지연 {delay}',
  fx:(S,n,a,v)=>K.delay(S,n,v.delay)},
...
/* 부품 한 칸. 설치물을 새로 놓지는 못한다 — canPart 가 rig>0 을 요구하므로
   「거치」가 먼저다.
   ★ 처음에는 kw 를 「설치」로 두고 rigSet 을 그대로 불렀다. 그러면 이 카드가
     설치 카드와 같은 규칙(수치 상한)으로만 막혀서, 「설치물마다 받는 부품 수가
     있다」는 말이 어디에도 없는 말이 된다. 키워드를 따로 세우고 붙이는 자도
     따로 세웠다 (카드.설치 — canPart · rigAddPart).
   칸이 차도 못 내는 카드가 되지는 않는다 — 첫 부품이 떨어지고 자리를 내준다.
   수치 상한은 처음 놓은 카드가 정한 n.rigCap 을 그대로 쓴다. 부품이 제 상한을
   따로 갖지 않는 까닭: 그러면 4짜리 거치 위에 2짜리 상한이 얹혀, 같은 자리의
   상한이 무엇을 마지막에 냈는가로 달라진다.
   빌려온 물건(rigLent)은 대상이 아니다 — 그것은 「강화되지도 개방되지도 않는다」 */
'덧대기': {cost:0, dept:'의공학', verb:'억제', kw:'부품', target:'node', v:{rig:2},
  need:n=>canPart(n),
  /* 본문에 「부품」을 적어 두는 까닭: 카드의 kw 는 어디에도 안 그려진다.
     키워드가 사람에게 닿는 길은 **본문에 그 낱말이 있는 것** 하나뿐이다
     (markKw 가 본문에서 KWORDS 를 찾아 밑줄을 긋는다). 「설치 +2」로만 두면
     새로 세운 부품 설명을 이 카드에서는 영영 못 읽는다.
     「사혈 1단 · 억제 −14」(매듭 짓다)와 같은 꼴이다 */
  text:'부품 · 설치 +{rig}',
  fx:(S,n,a,v)=>{ rigAddPart(S,n,v.rig) }},
