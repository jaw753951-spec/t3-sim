...
/* 이미 선 설치물에 부품을 덧댄다. 새로 놓지는 못한다 —
   need 가 rig>0 을 요구하므로 「거치」가 먼저다.
   상한은 처음 놓은 카드가 정한 n.rigCap 이다 (카드.설치). 덧대기가 제 상한을
   따로 갖지 않는 까닭: 그러면 4짜리 거치 위에 2짜리 상한이 얹혀, 같은 자리의
   상한이 무엇을 마지막에 냈는가로 달라진다.
   빌려온 물건(rigLent)은 대상이 아니다 — 그것은 「강화되지도 개방되지도 않는다」 */
'덧대기': {cost:0, dept:'의공학', verb:'억제', kw:'설치', target:'node', v:{rig:2},
  need:n=>n.rig>0 && canRig(n, CARDS['덧대기'].v.rig),
  text:'이미 선 설치물에 설치 +{rig}',
  fx:(S,n,a,v)=>{ rigSet(S,n,v.rig) }},
