...
## 4. 병 노드의 박자 — `BEATDOC` · `UNIQDOC`

병기 링 꼭대기의 「다음 박자」 그림에 뜬다. `고유` 는 보스 · 병기별로 `UNIQDOC` 이 대신한다.
...
const BEATDOC = {
  ...
  '고유':   {label:'이 병만의 한 수',
    why:()=>`고른 병 노드마다 다르다. 헛돌면 「성장」으로 대신한다.<br><br>` + uniqList()},
  '같은 박자': {label:'쉰다',
  ...
  '진행':   {label:'병기 시계를 당긴다',
    why:()=>`시계를 <b>${SR.BEAT_CLOCK}</b> 당긴다. 그 턴의 자연 감소 1 과 겹쳐 합쳐 ${SR.BEAT_CLOCK+1} 이 된다.<br><br><span class="d">「${BEAT_ALIAS.join('」·「')}」라 적어도 같은 비트다.</span>`},
...
```js
const UNIQDOC = {
  '아이:3': {name:'파고든다', why:()=>`통증과 호흡곤란의 진화 시계를 ${SR.BEAT_EVO_PULL}씩 당긴다.`},
  '아이:4': {name:'알아듣지 못한다', why:()=>'정신 상태가 한 단계 무너진다.'},
  '아이:5': {name:'터진다', why:()=>'휴면 상태의 증상 하나가 초기값으로 부활한다.'},
  '어부:3': {name:'긁는다', why:()=>`활성 통증 자리가 ${SR.BEAT_SCRATCH_N} 이상이고 감염이 없으면 이차 감염이 새로 앉는다.`},
  '송이:3': {name:'지금이면 괜찮아진다', why:()=>'휴면 상태의 증상 하나를 부활시키고 정신을 한 단계 무너뜨린다.'},
}
```
