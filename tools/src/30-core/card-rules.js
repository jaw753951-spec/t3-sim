...
/* 한 자리에 설치물 칸이 둘이다.
   ...
   ■ 값이 두 겹이다
     rigBase   설치 카드가 올린 값. **수치 상한(rigCap)은 이쪽만 잡는다.**
     rigParts  붙은 부품 목록. 상한을 안 탄다 — 칸 수가 곧 그 한계다.
     rig       둘의 합. 매 턴 깎는 값이고, 개방이 태우는 값이다.
   합을 rig 에 그대로 두는 까닭: 이 값을 읽는 곳이 여럿인데(턴 해결의 자동 억제 ·
   개방 · 계기 배지 · 탐색) 그 전부가 「이 설치물이 내는 값」을 뜻한다. 이름을
   옮기면 그 자리들을 다 따라다녀야 하고, 하나만 놓치면 조용히 옛 값을 읽는다.

   ★ 전에는 부품도 상한에 걸렸다. 그러면 거치(4)의 상한 12 를 부품 넷이 딱 채워서,
     칸이 찬 뒤의 교체가 값으로는 늘 0 이 됐다 — 규칙은 도는데 눈에 안 보였다.
     상한은 설치물이 제 힘으로 얼마나 커지는가의 한계지, 거기 매단 부품까지
     묶는 자가 아니다.

   자리는 붙은 부품을 **목록**으로 든다 (rigParts). 세는 것이 아니라 적는 까닭:
   떨어져 나갈 때 그 부품이 올려 준 값을 도로 빼야 하는데, 수만 세면 그 값을
   알 수 없다. 지금은 부품이 한 종류뿐이라 다 +2 지만, 값이 다른 부품이 생기면
   수로는 못 되돌린다.
...
/* 설치물이 지금 내는 값 = 설치분 + 붙은 부품. 값을 쓰는 자리는 rig 를 읽지만,
   놓고 붙이는 자리는 이 하나로 다시 센다 — 두 겹을 손으로 더하면 곧 갈린다 */
const rigSum = n => (n.rigBase||0) + (n.rigParts||[]).reduce((a,b)=>a+b, 0);
/* 이 자리에 amt 짜리 설치를 더 얹을 수 있는가 — rigSet 과 같은 잣대를 쓴다.
   보는 것은 **설치분**이다. 부품은 상한을 안 타므로 여기 안 든다 */
const canRig = (n, amt) => !(n.rig>0 && (n.rigBase||0) >= (n.rigCap || rigCapOf(amt)));

/* 부품 하나를 붙였을 때의 결과. **붙이는 자와 화면이 이 하나를 본다** —
   미는 셈을 화면이 따로 흉내 내면 곧 갈린다 (카드.팩 의 packOut 과 같은 잣대).
     off  떨어져 나간 부품의 값 (없으면 null)
   부품은 상한에 안 걸리므로 적힌 값이 그대로 올라간다 */
function partPreview(n, amt){
  if(!n || !(n.rig>0)) return null;
  const parts = (n.rigParts||[]).slice();
  let off = null;
  /* 칸이 찼으면 가장 먼저 붙인 것이 떨어진다. 그것이 올려 준 값도 함께 빠진다 */
  if(parts.length >= (n.rigPartMax||0)) off = parts.shift();
  parts.push(amt);
  return {parts, off, add:amt, rig:(n.rigBase||0) + parts.reduce((a,b)=>a+b, 0)};
}
...
function rigSet(S,n,amt){
  if(n.rig>0){
    ...
    if((n.rigBase||0) >= cap) return false;
    /* 설치는 부품 칸을 안 먹는다 — 설치물을 더 키우는 것이지 붙이는 것이 아니다.
       상한이 잡는 것도 이 설치분뿐이다 */
    n.rigBase = Math.min(cap, (n.rigBase||0) + amt);
  } else {
    n.rigBase = amt; n.rigCap = rigCapOf(amt);
...
function rigAddPart(S,n,amt){
  ...
  if(!p) return false;
  n.rigParts = p.parts; n.rig = rigSum(n);
  K.ev(S,{t:'rig', n, amt:n.rig});
  return true;
}
...
function rigOpen(S,n,mult){
  ...
  n.rig=0; n.rigBase=0; n.rigCap=0; n.rigParts=[]; n.rigPartMax=0;
  return got;
}
