...
/* ── 지연 ── 진화를 N턴 미루고 미룬 턴 1당 자리를 **지금 수치**의 10% 키운다 ──
   ★ 전에는 초기값 기준이었다. 그래서 자리를 처치선 코앞까지 깎아 놓은 뒤에도
     지연 한 번이 늘 같은 값을 되돌려 놨고 — 수치 12 짜리 자리에 초기값 90 의
     10% 인 9 가 얹혔다 — 다 잡아 놓은 자리에 지연을 거는 것이 가장 비쌌다.
     지금 수치를 보면 반대가 된다: 깎아 둔 자리는 미루는 값이 싸고, 부풀어 오른
     자리는 미루는 값이 비싸다. 미루는 대가가 판을 따라간다.

   미룬 턴만큼 곱하되 **복리로 굴리지 않는다** (val×1.1 을 k 번이 아니라
   ceil(val×0.1)×k). 지연은 카드를 낼 때 한 번에 얹히는 것이지 턴마다 다시
   얹히는 것이 아니라, 굴릴 「그때」가 없다. 굴리려면 지연이 남은 동안 매 턴
   턴해결이 다시 얹는 꼴로 옮겨야 하고 그것은 다른 규칙이다.

   상한은 그대로 초기값 기준이다 (VAL_CAP) — 그것은 지연 규칙이 아니라
   모든 자리에 걸리는 수치 천장이다.

   얹은 값을 자리에 적어 둔다. 지금 수치에서 나오는 값이라 **나중에 되짚을 수
   없기 때문이다** — 초기값 기준일 때는 화면이 ceil(init×0.1)×delayed 로 다시
   셈해서 「수치 +N」을 적었는데, 그 식이 여기와 두 벌이 되는 자리이기도 했다.
   이제 화면은 여기서 적어 둔 것을 읽는다 (화면.상태판). */
//@ 커널.지연 — 진화를 미루고 그만큼 지금 수치를 키운다
function delay(S,n,k){
  if(n.dead || immune(S,n)) return 0;
  n.evoLeft += k;
  n.delayed = (n.delayed||0) + k;
  const add = Math.ceil(n.val * R.DELAY_GROW) * k;
  n.val = Math.min(Math.floor(n.init*R.VAL_CAP), n.val + add);
  n.delayAdd = (n.delayAdd||0) + add;
  ev(S,{t:'delay', n, add});
  return add;
}
...
function evolveNow(S, n){
  ...
  ev(S,{t:'evolve', n});
  hurtPatient(S, Math.ceil((R.EVO_HIT[n.sym]||0)*policyDmg(S)), 'evo', n);   // v26 — 수치를 보지 않는 고정값
...
function transmit(S, src, L, k, strong){
  ...
  const nn = {sym:L.b, init, val:init, shielded:true, shReduc:R.SHIELD_CUT, stabAcc:0,
              ...
              evolved:false, dead:false, dormT:0, rig:0, rigBase:0, rigParts:[], rigPartMax:0, rigCap:0, rigLent:0, delayed:0, weak:0,
...
function turnResolve(S){
  ...
  if(!S.rem) for(const n of active(S))
    /* v26 — 때리는 몫은 turnDmg 가 낸다. 최대 피해가 0 인 증상은 0 을 돌려주므로
       「공격 증상인가」를 따로 묻지 않는다 — 무엇이 때리는지는 R.TURN_DMG 한 곳이 정한다 */
    if(n.role!=='disease' && !born(n)){
      const raw = turnDmg(n);
      if(raw>0){
        dmg += raw;
