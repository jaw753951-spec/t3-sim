...
function nodeMarks(S, n){
  ...
  return [
    ...
    /* 표식은 **합**을 보인다 — 매 턴 깎는 값이 그것이다. 두 겹으로 갈라 적는 것은
       설명에서 한다. 전에는 합을 설치 상한으로 나눠 적었는데, 부품이 상한을 안
       타게 되면서 「14/12」 같은 꼴이 나왔다 */
    n.rig?`<span class="m rig"${tip(TT('설치물',`매 턴 종료 시 이 자리를 <b>${n.rig}</b> 억제한다. 보호막을 무시한다.<br>설치 ${n.rigBase||0}/${n.rigCap||Math.max(R.RIG_CAP_MIN,n.rigBase||n.rig)}${n.rig-(n.rigBase||0)?` · 부품 +${n.rig-(n.rigBase||0)}`:''}<br>부품 ${(n.rigParts||[]).length}/${n.rigPartMax||0} — 부품은 설치 상한에 안 든다<br><br>개방하면 <b>−${n.rig*CARDS['출력 개방'].v.mult}</b> 한 방으로 태울 수 있다.`))}>설치물 ${n.rig}</span>`:'',
